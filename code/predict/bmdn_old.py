...
def Process_Final(Dataframe, aper:str):
    ...
    Training_Features = create_colors(broad=True, narrow=True, wise=True, galex=True)
    ...
    Dataframe = calculate_colors(Dataframe, broad=True, narrow=True, wise=True, galex=True)
    Training_Features = ['FUVmag-r_PStotal', 'NUVmag-r_PStotal', 'u_PStotal-r_PStotal', 'J0378_PStotal-r_PStotal', 'J0395_PStotal-r_PStotal', 'J0410_PStotal-r_PStotal', 'J0430_PStotal-r_PStotal', 'g_PStotal-r_PStotal', 'J0515_PStotal-r_PStotal', 'r_PStotal-J0660_PStotal', 'r_PStotal-i_PStotal', 'r_PStotal-J0861_PStotal', 'r_PStotal-z_PStotal', 'r_PStotal-W1_MAG', 'r_PStotal-W2_MAG']
...
def FinalPredict(Model:dict, Testing_Dataframe, Testing_Data_Features, Num_Samples=200):
    ...
    # Result_DF['z_bmdn_peak_2nd'] = Sec_ZPhot
    # Result_DF['pdf_width'] = PDF_STDs
    # Result_DF['odds_bmdn'] = Oddss
        
    for i in range(len(Final_Weights[0])):
        Result_DF[f'z_bmdn_pdf_weight_{i}'] = np.round(Final_Weights[:,i], 8)
    for i in range(len(Final_Means[0])):
        Result_DF[f'z_bmdn_pdf_mean_{i}'] = np.round(Final_Means[:,i], 8)
    for i in range(len(Final_STDs[0])):
        Result_DF[f'z_bmdn_pdf_std_{i}'] = np.round(Final_STDs[:,i], 8)
    ...
    #                               'n_peaks_bmdn': np.float32, 'z_bmdn_peak_2nd': np.float32,
